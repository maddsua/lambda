...
int lambda::virtualFS::saveSnapshot(std::string filepath) {
	...
	if (isGzipped) {
		stream.init(zlibCompressStream::defCompress, zlibCompressStream::header_gz);
	...
	for (auto& entry : vFiles) {
		...
		entryIdx++;
		auto fileStreamEnd = entryIdx >= vFiles.size();
		...
		if (isGzipped) {
			...
			stream.compressChunk(tarBuffer.data(), tarBuffer.size(), &gzBuffer, fileStreamEnd);
			if (stream.error()) return st_zlib_error;
		...
		} else localfile.write((const char*)tarBuffer.data(), tarBuffer.size());
