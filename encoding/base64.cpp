...
const uint8_t base64DecodeTable[256] = {
...
std::string Encoding::b64Decode(std::string& data) {
	...
	for (size_t ibase = 0, ibin = 0; ibase < encodedLength; ibase += 4, ibin += 3){
		//	byte 1/1.33
		result[ibin] = base64DecodeTable[(data)[ibase]] ^ (base64DecodeTable[(data)[ibase+1]] >> 6);
		//	byte 2/2.66
		result[ibin + 1] = (base64DecodeTable[(data)[ibase+1]] << 2) ^ (base64DecodeTable[(data)[ibase+2]] >> 4);
		//	byte 3/4
		result[ibin + 2] = (base64DecodeTable[(data)[ibase+2]] << 4) ^ (base64DecodeTable[(data)[ibase+3]] >> 2);
	}
...
std::string Encoding::b64Encode(std::string& data) {
	...
	for (size_t ibin = 0, ibase = 0; ibin < contentLength; ibin += 3, ibase += 4) {
		//	byte 1/0.75
		result[ibase] = base64EncodeTable[((data)[ibin] >> 2)];
		//	byte 2/1.5
		result[ibase + 1] = base64EncodeTable[(((((data)[ibin] << 4) ^ ((data)[ibin + 1] >> 4))) & 0x3F)];
		//	byte 3/2.25
		result[ibase + 2] = base64EncodeTable[(((((data)[ibin + 1] << 2) ^ ((data)[ibin + 2] >> 6))) & 0x3F)];
		//	byte 4/3
		result[ibase + 3] = base64EncodeTable[((data)[ibin + 2] & 0x3F)];
	}
...
bool Encoding::b64Validate(const std::string& data) {
